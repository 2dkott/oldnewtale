...
public class PhysicSystem extends IteratingSystem implements EntityListener, ContactListener {
	...
	public PhysicSystem(World world, float interval) {
		super(Family.all(PhysicComponent.class, TransformComponent.class).get());
		this.world = world;
	...
	protected void processEntity(Entity entity, float deltaTime) {
		...
		physic.getCurrentPosition().set(physic.getBody().getPosition());
	}
	...
	private void interpolateEntity(Entity entity, float alpha) {
		...
		transform.getPosition().set(
				MathUtils.lerp(physic.getCurrentPosition().x, physic.getBody().getPosition().x, alpha),
				MathUtils.lerp(physic.getCurrentPosition().y, physic.getBody().getPosition().y, alpha));
	}
	...
	public void entityRemoved(Entity entity) {
		...
		// This ONLY works when an entity with a Physic component gets removed entirely
		// from the engine.
		PhysicComponent physic = PhysicComponent.MAPPER.get(entity);
		if (physic != null) {
